...
describe("RoleEmailTable", () => {
  ...
  test("Check that api endpoint overrides work", async () => {
    axiosMock.onDelete("/api/admin/moderator/delete").reply(200, {});
    ...
    render(
      <QueryClientProvider client={queryClient}>
        <MemoryRouter>
          <RoleEmailTable
            data={testData}
            deleteEndpoint="/api/admin/moderator/delete"
            getEndpoint="/api/admin/moderator/all"
          />
        </MemoryRouter>
      </QueryClientProvider>,
    );
    ...
    expect(axiosMock.history.delete[0].url).toBe("/api/admin/moderator/delete");
    expect(axiosMock.history.delete[0].params).toEqual({
      email: "user1@example.org",
    });
